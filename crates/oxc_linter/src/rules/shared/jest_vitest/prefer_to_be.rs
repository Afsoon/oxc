...
pub fn run<'a>(possible_jest_node: &PossibleJestNode<'a, '_>, ctx: &LintContext<'a>) {
    ...
    if has_not_modifier {
        if matcher.is_name_equal("toBeUndefined") {
            check_and_fix(&PreferToBeKind::Defined, call_expr, matcher, &jest_expect_fn_call, ctx);
            return;
    ...
    let Some(arg_expr) = jest_expect_fn_call.args.first().and_then(Argument::as_expression) else {
        return;
    };
    ...
    if first_matcher_arg.is_undefined() {
        ...
        check_and_fix(&kind, call_expr, matcher, &jest_expect_fn_call, ctx);
        return;
    }

    if first_matcher_arg.is_nan() {
        check_and_fix(&PreferToBeKind::NaN, call_expr, matcher, &jest_expect_fn_call, ctx);
        return;
    }

    if first_matcher_arg.is_null() {
        check_and_fix(&PreferToBeKind::Null, call_expr, matcher, &jest_expect_fn_call, ctx);
        return;
    }

    if should_use_tobe(first_matcher_arg)
        && !matcher.is_name_equal("toBe")
        && !should_skip_float(first_matcher_arg, ctx)
    {
        check_and_fix(&PreferToBeKind::ToBe, call_expr, matcher, &jest_expect_fn_call, ctx);
    }
}
...
fn check_and_fix(
    ...
    if kind == &PreferToBeKind::Undefined {
        ...
        let new_matcher = if is_cmp_mem_expr { "[\"toBeUndefined\"]()" } else { "toBeUndefined()" };
        ...
        let (source_start, suggestion) = build_suggestion_with_not_modifier(
            "toBeNull",
            maybe_not_modifier,
            is_cmp_mem_expr,
            span.start,
